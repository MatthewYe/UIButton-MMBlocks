...
//static char tagArrayKey;

#pragma mark - Public
- (void)addAction:(ButtonBlock)block forControlEvent:(UIControlEvents)controlEvent{
    if (controlEvent <= 1<<8) {
...
- (NSString*)blocksArrayKeyForControlEvent:(UIControlEvents)controlEvent{
    return [NSString stringWithFormat:@"tagArrayKey%lu",(unsigned long)controlEvent];
}
- (NSMutableArray*)actionsArrayForControlEvent:(UIControlEvents)controlEvent{
   ...
   NSMutableArray * tags = (NSMutableArray *)objc_getAssociatedObject(self, key);
    if (!tags) {
        ...
        objc_setAssociatedObject(self, key, tags,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return tags;
}
